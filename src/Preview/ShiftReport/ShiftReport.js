   ...
   return (
      <section className={`${style.warp} shift-warp`}>
         <header className={`${style.header} shift-header`}>
            ...
            {/* confirm */}
            <div className={`${style.confirmImg} shift-confirmImg`}>{state?.confirm?.status && confirm}</div>
         </header>
         <div className={`${style.timeStamp} shift-timeStamp`}>
            {state?.date?.session}, Ngày {state?.date?.date} Tháng {state?.date?.month} năm {state?.date?.year}
         </div>
         <section style={{ width: '100%', textAlign: 'center' }}>
            <table className={`${style.table} shift-table`}>
               <caption className={`${style.tableCaption} shift-tableCaption`}>I. Các công việc/Sự cố trong Ca</caption>
               <thead>
                  <tr>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '4%' }}>
                        Stt
                     </th>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '15%' }}>
                        Khu vực
                     </th>
                     <th className={`${style.tableTitle} shift-tableTitle`}>Nội dung</th>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '13%' }}>
                        Thời gian
                     </th>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '15%' }}>
                        Kết quả
               ...
               {state?.issue.map((crr, index) => {
                  return (
                     <tbody key={index}>
                        <tr>
                           <td className={`${style.tableContent} shift-tableContent`}>{index + 1}</td>
                           ...
                           <td className={`${style.tableContent} shift-tableContent`}>{crr?.time}</td>
                           <td className={`${style.tableContent} shift-tableContent`}>{crr?.result}</td>
                        </tr>
                     </tbody>
                  );
               })}
            ...
            {/* /// */}
            <table className={`${style.table} shift-table`}>
               <caption className={`${style.tableCaption} shift-tableCaption`}>II. KSKV/ CA trước giao việc</caption>
               <thead>
                  <tr>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '4%' }}>
                        Stt
                     </th>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '22%' }}>
                        Người giao
                     </th>
               ...
               {state?.order.map((crr, index) => {
                  return (
                     <tbody key={index}>
                        <tr>
                           <td className={`${style.tableContent} shift-tableContent`}>{index + 1}</td>
                           <td className={`${style.tableContent} shift-tableContent`}>{crr?.people}</td>
                           <td
                              className={`${style.tableContent} shift-tableContent`}
                              dangerouslySetInnerHTML={{ __html: crr?.content?.replace(/\n/g, '<br>') }}
                           ></td>
                        </tr>
                     </tbody>
                  );
            ...
            {/* /// */}
            <table className={`${style.table} shift-table`}>
               <caption className={`${style.tableCaption} shift-tableCaption`}>III. Ý kiến/Đề xuất</caption>
               <thead>
                  <tr>
                     <th className={`${style.tableTitle} shift-tableTitle`} style={{ width: '4%' }}>
                        Stt
                              ...
                              return (
                                 <tbody key={index}>
                                    <tr>
                                       ...
                                       <td className={`${style.tableContentChild} shift-tableTitleChild`}>{crr?.IDCode}</td>

                                       <td className={`${style.tableContentChild} shift-tableTitleChild`}>{crr?.name}</td>
                                       <td className={`${style.tableContentChild} shift-tableTitleChild`}>{crr?.amount}</td>
                                       <td className={`${style.tableContentChild} shift-tableTitleChild`}>{crr?.unit}</td>
                                    </tr>
                                 </tbody>
                              );
                           })}
                     ...
                     <tr>
                        ...
                        {state?.confirm?.status ? (
                           ...
                           <td className={`${style.tableSignContent} shift-tableSignContent `}>
                              ...
                              <div style={{ fontSize: '0.6rem', fontStyle: 'italic', color: '#ccc' }}>{state?.confirm?.message}</div>
                           </td>
                        ) : (
                           // *TODO: nếu Chưa xác nhận thì hiện mã này
                           // Tiếp tục check điều kiện Auth khác current_user

                           <td className={`${style.tableSignContent} shift-hidden shiftConfirm`} style={{ display: 'flex', flexDirection: 'column' }}>
                              {state?.authEmail === currentUser || currentUser === 'none' ? (
                                 ...
                                 <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <a className={`${style.tableSignContentLoginConfirm} `} href="/login">
                                       
                                       Đăng nhập để xác nhận
                                    </a>
                                    <div style={{fontSize: '10px', fontStyle: 'italic', color: 'gray'}}>Người giao và người nhận phải khác nhau !!!</div>
                                 </div>
                              ) : (
                                 // *TODO: nếu khác user và đã đăng nhập thì hiện nút để nhán xác nhận
                                 <div>
                                    <button className={`${style.LoginConfirmButton} `} onClick={handelLoginConfirm}>
                                       Xác Nhận
                                    </button>
